#!/usr/bin/env python3
"""
Modified GeoJSON cleanup script that:
...
def clean_geojson(input_file, output_dir=None):
    ...
    # Fields to remove (updated list including Google Earth export fields)
    fields_to_remove = {
        'text', 'anchor', 'icon', 'tooltip', 'textPosition', 'stroke', 'rotate', 'offsetY',
        'offsetX', 'locked', 'marker-size', 'labelStyle', 'styleUrl', 'fill-opacity', 'fill',
        'stroke-opacity', 'stroke-width', 'icon-offset', 'icon-offset-units'
    }
...
def main():
    """
    Main CLI entry point for geojson_cleanup_no_wiki.py.
    Parses arguments and runs the cleaning process.
    """
    parser = argparse.ArgumentParser(
        description='Clean up GeoJSON files with unicode fixes, duplicate detection, field removal, and empty Wikipedia fields.',
